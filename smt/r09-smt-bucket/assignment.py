...
def goal_state_formula(bucket_vars_at_i, goal: int):
    ...
    return Or([bucket_var == goal for bucket_var in bucket_vars_at_i])
...
def step_formula(bucket_capacities,
    ...
    #
    for bucket in range(0, nof_buckets):
      act = bucket_vars_at_next_i[bucket] == 0
      frame = And([(bucket_vars_at_next_i[other] == bucket_vars_at_i[other])
                    for other in range(0, nof_buckets) if other != bucket])
      constraints.append(Implies(empties_at_i[bucket], And(act, frame)))


    #
    # Finally, the same for pours_at_i
    #
    for bucket1 in range(0, nof_buckets):
      for bucket2 in range(0, nof_buckets):
        if bucket1 != bucket2:
          act2 = If(bucket_vars_at_i[bucket1] + bucket_vars_at_i[bucket2] <= bucket_capacities[bucket2],
                    bucket_vars_at_next_i[bucket2] == bucket_vars_at_i[bucket1] + bucket_vars_at_i[bucket2],
                    bucket_vars_at_next_i[bucket2] == bucket_capacities[bucket2])
          
          act1 = bucket_vars_at_next_i[bucket1] == bucket_vars_at_i[bucket1] + bucket_vars_at_i[bucket2] - bucket_vars_at_next_i[bucket2]
          

          frame = And([(bucket_vars_at_next_i[other] == bucket_vars_at_i[other])
                        for other in range(0, nof_buckets) if other != bucket1 and other != bucket2])
          
          constraints.append(Implies(pours_at_i[bucket1][bucket2], And(act1, act2, frame)))
