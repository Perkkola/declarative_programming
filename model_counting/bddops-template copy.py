...
class OBDD():
    ...
    def newOBDDnode(self,rootVar,child1,child2):
        ...
        i = self.varIndex[rootVar]
        nvars = len(self.vars)

        if isinstance(child1, int):
            child1_index = nvars
            base_count1 = 1 if child1 == 1 else 0
        else:
            child1_index = self.varIndex[child1.nodeVar]
            base_count1 = child1.modelCount

        if isinstance(child2, int):
            child2_index = nvars
            base_count2 = 1 if child2 == 1 else 0
        else:
            child2_index = self.varIndex[child2.nodeVar]
            base_count2 = child2.modelCount

        gap1 = child1_index - i - 1
        gap2 = child2_index - i - 1
        count1 = base_count1 * (2 ** gap1)
        count2 = base_count2 * (2 ** gap2)
